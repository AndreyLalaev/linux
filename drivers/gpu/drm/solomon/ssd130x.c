...
static void ssd130x_reset(struct ssd130x_device *ssd130x)
{
	...
	dev_info(ssd130x->dev, "resetting ssd1351\n");
	/* Reset the screen */
...
static int ssd135x_init(struct ssd130x_device *ssd130x)
{
	 ...
	 * 65k color format
	 */
	ret = ssd130x_write_cmd(ssd130x, 2, SSD13XX_SET_SEG_REMAP, 0x34);
	if (ret < 0)
		return ret;
	...
	/* Set Current ATT */
	ret = ssd130x_write_cmd(ssd130x, 2, 0xC7, 0x0f);
	if (ret < 0)
		return ret;
	...
	/* Disable scrolling */
	ret = ssd130x_write_cmd(ssd130x, 1, 0x9e);
	if (ret < 0)
		return ret;
